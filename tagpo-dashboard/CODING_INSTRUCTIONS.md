...
2. **App.jsx**: `dbToFront`/`frontToDb`に`review`フィールド追加済み。フォームに審査入力欄、詳細表示に審査表示も追加済み。

3. **テーブル一覧行にも審査を表示**: ステータスバッジの横に、`review`の値があれば小さいテキストで表示する。ステータスの`<span>`（バッジ）の直後に、`c.review`が空でなければ表示。スタイルは控えめに（fontSize: 10, color: "#64748b" 程度）。

   変更箇所: テーブル行のステータス列（`{c.status}`のバッジを表示している`<span>`のすぐ後）
