...
- **縦軸**: 案件（メーカー名 / 商品名）。フィルター適用後の案件が並ぶ
- **マイルストーン表示**: 各案件の行に、6つのマイルストーン（ES回収〜レポート送付）をそれぞれ丸印やマーカーで該当日付の位置に配置
  - ✓済みマイルストーン: 緑の丸（`#10b981`）
  - 遅延中: 赤の丸（`#ef4444`）
  - 未到達（日付あり）: 青の丸（`#3b82f6`）
  - 日付未設定: 表示しない
- **投稿期間バー**: `postStart`〜`postEnd`の期間を薄い横棒で表示（これが「ガント」っぽさを出す）
- **今日の線**: 今日の日付に縦の赤い点線を引く

**インタラクション**:
- マイルストーンの丸にホバーすると、ツールチップで「ES回収 3/1 ✓済み」等の詳細を表示
- 案件名クリックで行を展開（テーブルビューと同じ詳細パネル）
- 横スクロール可能（期間が長い場合）

**スタイル**:
- 行の背景色はテーブルビューと同じステータスベースの色分け
...
- [ ] 全案件のマイルストーンがガントチャート上の正しい日付位置に表示されていること
- [ ] 今日の線が正しい位置に表示されていること
- [ ] ✓済み/遅延中/未到達のマイルストーンが色分けされていること
- [ ] 投稿期間（postStart〜postEnd）の横棒が表示されていること
- [ ] フィルター（ステータス絞り込み）がカレンダービューにも適用されること
- [ ] 横スクロールで全期間が閲覧できること
- [ ] レスポンシブ: 狭い画面でも横スクロールで使えること
