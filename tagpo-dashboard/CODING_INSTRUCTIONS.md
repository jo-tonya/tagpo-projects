...
## Phase 3.5: 「指定小売店」フィールド追加

### 目的
案件ごとに指定小売店（例: ウエルシア、ツルハ等）を記録できるようにする。

### タスク

1. **DBマイグレーション**: `campaigns`テーブルに`retailer`カラム（text, nullable）を追加
   ```sql
   ALTER TABLE campaigns ADD COLUMN retailer text;
   ```

2. **App.jsx の変更**:

   **a) `dbToFront` に追加**（`review`の近く）:
   ```js
   retailer: row.retailer || "",
   ```

   **b) `frontToDb` に追加**（`review`の近く）:
   ```js
   retailer: c.retailer || "",
   ```

   **c) `CampaignForm` の `useState` 初期値に追加**:
   ```js
   retailer:"",
   ```

   **d) フォームUI**: Basicセクション（メーカー名・商品名・ステータス・既存/新商品・審査の並び）に「指定小売店」を追加。短いテキスト入力欄。配置は**前の方**（メーカー名・商品名の次の行、ステータスや審査と同じ行）。
   ```jsx
   ...
   **e) テーブル一覧行に独立した列として追加**: ステータス・審査と同じ並びに「小売店」列を追加（ヘッダーとデータ行の`gridTemplateColumns`にも列幅を追加）。値が空なら「—」を表示。

   **f) 展開時の詳細表示**: 案件詳細にも表示（タイプ・審査の近く）:
   ```jsx
   <DI l="指定小売店" v={c.retailer || "—"} />
   ```

### 動作検証
- [ ] Supabaseで`campaigns`テーブルに`retailer`カラムが存在すること
- [ ] 案件編集で「指定小売店」に入力 → 保存 → リロード後も値が保持されていること
- [ ] テーブル一覧に「小売店」列が表示され、値が入っていること
- [ ] 展開時の詳細にも表示されていること
